...
public class AdminController {
	...
	public String adminLogin(AdminVO vo, Model model) {
		...
		// (2) 정상 관리자이면 
		//     - 관리자 정보 조회
		//     - 상품목록화면 으로 이동 (redirect:admin_product_list)
		if (result == 1) {
	...
	*/
	...
	public String adminProductDetailAction(ProductVO vo, Criteria criteria, Model model) {
		...
		System.out.println("[AdminController: adminProductDetailAction() criteria="+criteria);
	...
	public String adminProductUpdate(@RequestParam(value="product_image") MultipartFile uploadFile,
		...
		if (!uploadFile.isEmpty()) {  // 상품 이미지 수정한 경우
			String fileName = uploadFile.getOriginalFilename();
			vo.setImage(fileName);
